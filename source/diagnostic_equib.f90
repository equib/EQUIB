...
!                                                                       
     ...
     & RLINT, FINT, SUMA, SUMB, SUMC, QX, AX, EX, FRAT, DEE
      ...
      REAL*8 inratio, fixedq
      CHARACTER*20 inratioch,fixedqch,levelu,levell
      ...
      READ(levelu,*) ((ITRANA(LL,KK),LL=1,2),KK=1,150)
      READ(levell,*) ((ITRANB(LL,KK),LL=1,2),KK=1,150)

      IONL = INDEX(ION,' ') - 1
      OPEN(UNIT=1,STATUS='OLD',                                         &
     & FILE='atomic-data/'//ION(1:IONL)//'.dat') 
                                                      !Read in no. comment lines
      !print*,'/media/linux/idlfortran/Atomic-data/'//ION(1:IONL)//'.dat'
      READ(1,*) NLINES
      DO I = 1, NLINES
                                                                       !Comments
        READ(1,1003) LTEXT 
...
! set up T and D loops depending on input.
      ...
      if (diagtype .eq. "t" .or. diagtype .eq. "T") then
        ...
        IND=4
        DINC=(100000.)/((IND-1)**(LOOP)) !DINC=(100000.)/((INT-1)**(LOOP))
      ...
      if (densi .le. 0) densi=1
          if (tempi .lt. 5000) tempi=5000 !add
...
!        enddo
      ...
      print "(3X,F6.0,21X,F6.0)",valtest(1:2)
